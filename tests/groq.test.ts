...
describe('cleanGroqError', () => {
  it('extracts OpenAI-compatible JSON error message', () => {
    ...
    const msg = cleanGroqError(401, 'Unauthorized', raw);
...
<html>
<head><title>groq.com | 502: Bad gateway</title></head>
<body><div id="cf-wrapper"><h1>Bad gateway</h1></div></body>
</html>`;
    const msg = cleanGroqError(502, 'Bad Gateway', html);
  ...
  it('sanitizes and truncates long plain text', () => {
    ...
    const msg = cleanGroqError(500, 'Internal Server Error', text);
...
describe('callGroqChatCompletions', () => {
  it('returns parsed JSON on 200 OK', async () => {
    ...
    const res = await callGroqChatCompletions({
  ...
  it('retries on transient 502 and succeeds on subsequent attempt', async () => {
    const cfHtml = '<!DOCTYPE html><title>groq.com | 502: Bad gateway</title>';
    const mockFetch = vi
      .fn()
  ...
  it('throws clean GroqError when all retries fail with 502', async () => {
    const cfHtml = '<!DOCTYPE html><title>groq.com | 502: Bad gateway</title><body>Bad gateway</body>';
